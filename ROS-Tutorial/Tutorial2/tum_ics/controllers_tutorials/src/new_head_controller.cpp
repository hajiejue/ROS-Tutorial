...
class CombinedResourceController_new : public controller_interface::ControllerBase
{
public:
  ...
  void update(const ros::Time& time, const ros::Duration& period)
  {
    ...
    if( (time.nsec % 10000000) == 0)   // Coment this line when working on the real robot
    {
     ...
     // double pos = 0.0;     // For HSRB
      ...
      joints_[1].setCommand(0.2*sin(cont) + pos);
...
PLUGINLIB_DECLARE_CLASS(controllers_tutorials, CombinedResourceController_new, controllers_tutorials::CombinedResourceController_new, controller_interface::ControllerBase);
